%% droop_model.slx Data
clear all
%% Solver sample time
T_s=10e-5;%Increasing Ts makes simulation run faster, but it may increase the numerical error as well!
Tend=50;
T_loss=Tend;
T_load=25;
...
I_b_LV=sqrt(2)*S_b/(sqrt(3)*V1_rms);
V_m=sqrt(2/3)*V1_rms;
Vdc_n=3*V_m;
n=200;
C_dc=0.008*(n);
...
base=2.25; % base load
load_change=0.9;% load disturbance
